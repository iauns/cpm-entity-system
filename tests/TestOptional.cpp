...
int BasicSystem::numNulls = 0;

TEST(EntitySystem, BasicOptionalTest)
{
  ...
  // This is *not* an invalid component. homPos is an optional component.
  id = core->getNewEntityID();
  core->addComponent(id, posComponents[id]);
  core->addComponent(id, gameplayComponents[id]);
  ...
  EXPECT_EQ(1, BasicSystem::numNulls);
}
