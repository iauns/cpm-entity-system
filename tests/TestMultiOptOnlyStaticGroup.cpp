...
class BasicSystem : public es::GenericSystem<true, CompStaticLightDir, CompPosition, 
...
public:
                    ...
                    const es::ComponentGroup<CompGameplay>& gp) override
  {
    ...
    if (homPos.numComponents != 0) homPos.components->component.checkEqual(homPosComponents[entityID]);
    if (pos.numComponents != 0) pos.components->component.checkEqual(posComponents[entityID]);
...
TEST(EntitySystem, MultiOptionalOnlyStaticGroupTest)
{
