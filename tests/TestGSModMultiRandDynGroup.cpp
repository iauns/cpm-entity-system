...
// This class tests the entity system's modification system. The crux of the
// system is to avoid changing any entity's state *this* frame. Instead,
// changing it next frame.
...
// Component container that systems can use to check whether the components
...
struct ComponentItem
{
  ...
  // A list of modified component values that will be applied at the end of
  // the frame. Very similar to what is happening inside of the entity
  // system.
  struct ModifiedComponents
  {
    ModifiedComponents(uint64_t id, int index, const T& val, int pri) :
        entityID(id),
        listIndex(index),
        value(val),
        priority(pri)
  ...
  std::vector<ModifiedComponents> modifiedComponents;
  uint64_t highestEntityID;

  ComponentItem() : highestEntityID(0) {}

  void applyModifiedComponents()
  {
    std::sort(modifiedComponents.begin(), modifiedComponents.end(), modificationCompare);

    uint64_t lastEntityID = 0;
    int lastListIndex = -1;
    for (const ModifiedComponents& mod : modifiedComponents)
    {
      if (!(mod.entityID == lastEntityID && mod.listIndex == lastListIndex))
      {
        // Modify the item since we haven't touched this entity and listindex
        // before.
        std::list<T>* compList = getComponentList(mod.entityID);

        bool foundComp = false;
        int i = 0;
        for (auto it = compList->begin(); it != compList->end(); ++it, ++i)
        {
          if (i == mod.listIndex)
          {
            *it = mod.value;
        ...
        ASSERT_EQ(true, foundComp);
      }

      lastEntityID = mod.entityID;
      lastListIndex = mod.listIndex;
    }

    // Clear out all modified components now that they are no longer needed.
    modifiedComponents.clear();
  }

  static bool modificationCompare(const ModifiedComponents& a, const ModifiedComponents& b)
  {
    if (a.entityID < b.entityID) return true;
    if (b.entityID < a.entityID) return false;

    // a == b for entityID, now sort based on listIndex.
    if (a.listIndex < b.listIndex) return true;
    if (b.listIndex < a.listIndex) return false;

    // a == b for listIndex, now sort based on priority -- ascending.
  ...
  void addModifiedComponent(uint64_t entityID, const T& value, int index, int priority)
  {
    modifiedComponents.emplace_back(entityID, index, value, priority);
  }
...
class TestSystem : public es::GenericSystem<true, Ts...>
{
public:

  TestSystem(IntRandFun irand, DoubleRandFun drand, IntRandFun gen100) :
      mIRand(irand),
      mDRand(drand),
      mGen100(gen100)
  {}
  ...
  void groupExecute(es::ESCore&, uint64_t entityID, const es::ComponentGroup<Ts>&... groups) override
  {
    ...
    // We can do this more accurately than the recursive case since we know
    // this function will be called exactly once per entityID.

    int firstFailedIndex = GroupCheck<0, Ts...>::check(this, entityID, groups...);
    if (firstFailedIndex != -1)
    {
      // Do this so we can set a break.
      ASSERT_EQ(false, true) << "Failed index: " << firstFailedIndex << " failed ID: " << entityID << std::endl;
    }

    // Randomly modify given elements.
    ModifyComponents<0, Ts...>::modify(this, entityID, groups...);
  }

  template <int GroupIndex, typename... RTs>
  struct ModifyComponents;

  template <int GroupIndex, typename RT, typename... RTs>
  struct ModifyComponents<GroupIndex, RT, RTs...>
  {
    static void modify(TestSystem<Ts...>* ptr, uint64_t entityID,
                       const es::ComponentGroup<RT>& head,
                       const es::ComponentGroup<RTs>&... rest)
    {
      // Determine if we should modify components.
      if (ptr->mGen100() <= 15)
      {
        int i = 0;
        for (auto it = head.begin(); it != head.end(); ++it, ++i)
        {
          if (ptr->mGen100() < 70)
          {
            RT thisComponent = RT(ptr->mIRand, ptr->mDRand);
            ComponentItem<RT>::instance()->addModifiedComponent(entityID, thisComponent, i, 1);
            head.modify(thisComponent, i, 1);
          }
        }
      }
      ModifyComponents<GroupIndex + 1, RTs...>::modify(ptr, entityID, rest...);
    }
  };

  template <int GroupIndex>
  struct ModifyComponents<GroupIndex>
  {
    static void modify(TestSystem<Ts...>* /* ptr */, uint64_t /* entityID */) {}
  };
  ...
  template <int GroupIndex, typename RT, typename... RTs>
  struct GroupCheck<GroupIndex, RT, RTs...>
  {
    static int check(TestSystem<Ts...>* ptr, uint64_t entityID,
                     const es::ComponentGroup<RT>& head, 
                     const es::ComponentGroup<RTs>&... rest)
    {
      // Check head against ComponentItem<RT>::instance()'s data.
      std::list<RT>* list = ComponentItem<RT>::instance()->getComponentList(entityID);

      if (list->size() != head.numComponents)
      {
        std::cout << "Unequal sizes: " << list->size() << "," << head.numComponents << std::endl;
        return GroupIndex;
      }

      // Iterate through the list, and since we are using a stable sort, the
      // grouped items should be in the *exact* same order as the list.
      int index = 0;
      for (auto it = list->begin(); it != list->end(); ++it)
      {
        if (!head.components[index].component.checkEqual(&(*it)))
          return GroupIndex;
        ++index;
      }
...
TEST(EntitySystem, ModMultiRandDynGrouped)
{
  ...
  auto testSystemA = new TestSystem<CompPosition, CompHomPos, CompGameplay>(gen100, genReal, gen100);
  auto testSystemB = new TestSystem<CompHomPos, CompGameplay>(gen100, genReal, gen100);
  auto testSystemC = new TestSystem<CompGameplay, CompTest2>(gen100, genReal, gen100);
  auto testSystemD = new TestSystem<CompTest1, CompTest2>(gen100, genReal, gen100);
  auto testSystemE = new TestSystem<CompPosition, CompTest3, CompTest1, CompTest2>(gen100, genReal, gen100);
  auto testSystemF = new TestSystem<CompPosition, CompHomPos, CompGameplay, CompTest1, CompTest2, CompTest3>(gen100, genReal, gen100);
  auto testSystemG = new TestSystem<CompPosition, CompHomPos, CompTest1, CompTest2, CompTest3>(gen100, genReal, gen100);
  auto testSystemH = new TestSystem<CompPosition, CompHomPos, CompTest1, CompTest2>(gen100, genReal, gen100);
  auto testSystemI = new TestSystem<CompPosition, CompTest1, CompTest2>(gen100, genReal, gen100);
  auto testSystemJ = new TestSystem<CompTest1, CompTest3>(gen100, genReal, gen100);
  ...
  for (int i = 0; i < numIterations; ++i)
  {
    ...
    // Ensure we apply all modifications.
    c_pos->applyModifiedComponents();
    c_homPos->applyModifiedComponents();
    c_gameplay->applyModifiedComponents();
    c_test1->applyModifiedComponents();
    c_test2->applyModifiedComponents();
    c_test3->applyModifiedComponents();
