...
struct CompPosition
{
  ...
  // Compile time polymorphic functions:
...
TEST(EntitySystem, TestGetName)
{
  ...
  uint64_t componentID = es::TemplateID<CompPosition>::getID();
  es::BaseComponentContainer* cont = core->getComponentContainer(componentID);
  ASSERT_NE(nullptr, cont);
  EXPECT_EQ("render:CompPosition", std::string(cont->getComponentName()));

  componentID = es::TemplateID<CompHomPos>::getID();
  cont = core->getComponentContainer(componentID);
  ASSERT_NE(nullptr, cont);
  EXPECT_EQ("render:CompHomPos", std::string(cont->getComponentName()));

  componentID = es::TemplateID<CompGameplay>::getID();
  cont = core->getComponentContainer(componentID);
  ASSERT_NE(nullptr, cont);
  EXPECT_EQ("render:CompGameplay", std::string(cont->getComponentName()));
